...
class DocumentHandler:
    def __init__(self, store, chunk_size=1000, chunk_overlap=200):
    ...
    def extract_text_from_file(self, file_path):
        ...
        try:
            match file_extension:
                ...
                case _:
                    logger.warning(
                        f"[DOCUMENT HANDLER] {file_extension} is an unsupported document format"
                    )
        except Exception as ex:
            logger.warning(
                f"[DOCUMENT HANDLER] failed to parse: {file_path} with exception: ", ex
            )
    ...
    def train_from_document(self, doc_path: str, doc_id=None):
        ...
        try:
            ...
            print(ex)
            logger.warning(
                f"[DOCUMENT HANDLER] Failed to parse: {doc_path}. Error: ", ex
            )
